...
class ParcelOrdergetallPaginController extends GetxController {
  final PagingController<int, dynamic> getparcelOrderHistoryController =
  ...
  void onInit() {
    super.onInit();
    getparcelOrderHistoryController.addPageRequestListener((pageKey) {
      fetchHistoryPage(pageKey);
    });
  }

  Future<void> fetchHistoryPage(int pageKey) async {
    await getparcelOrdersHistory(
      pageKey,
      getparcelOrderHistoryController,
    );
  }

  Future<void> getparcelOrdersHistory(
      int pageKey, PagingController<int, dynamic> pagingController) async {
    try {
      var response = await http.get(
        Uri.parse(
            '${API.baseUrl}api/order/orderGetPagination/?subAdminType=services&userId=$UserId&limit=$perPage&offset=$pageKey&orderStatus=delivered,cancelled,rejected,new,orderAssigned,orderPickedUped,deliverymanReachedDoor,initiated'),
        headers: API().headers,
      );
  ...
  @override
  void onClose() {
    getparcelOrderHistoryController.dispose();
    super.onClose();
  }
}
