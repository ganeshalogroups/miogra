...
class MartAdditionalinfo extends StatefulWidget {
  const MartAdditionalinfo({super.key});

  @override
  State<MartAdditionalinfo> createState() => _MartAdditionalinfoState();
}


class MartAdditionalinfocheckboxController extends GetxController {
...
class MartAddDeliveryInstructions extends StatelessWidget {
  
  final VoidCallback onTapp;
  const MartAddDeliveryInstructions({super.key,required this.onTapp});
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              Consumer<MartInstantUpdateProvider>(
...
class _MartAdditionalinfoState extends State<MartAdditionalinfo> {
  MartAdditionalinfocheckboxController checkboxController = Get.put(MartAdditionalinfocheckboxController());
  ...
  void checkIFAlreadyCecked() {
    var upDates = Provider.of<MartInstantUpdateProvider>(context, listen: false);
  ...
  void _updateInstruction() {
    if (pendingInstruction != null) {
      Provider.of<MartInstantUpdateProvider>(context, listen: false).martupDateInstruction(instruction: pendingInstruction!);
      Get.back(); // Close the modal or page if needed
    } else {
      Provider.of<MartInstantUpdateProvider>(context, listen: false).martupDateInstruction(instruction: '');
      Get.back(); // Close the modal or page if needed
...
class MartInstantUpdateProvider with ChangeNotifier {
//Order For SomeOne
  String meatsomeName = '';
...
// Add Delivary Instruction
  ...
  martupdateSomeOneDetails({someOneName, someOneNumber}) {
    meatsomeName = someOneName;
    meatsomeNumber = someOneNumber;
    notifyListeners();
  }

  martupDateInstruction({instruction}) {
