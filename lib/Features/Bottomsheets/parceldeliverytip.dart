...
class ParcelDriverTipSelector extends StatefulWidget {
  final GlobalKey<FormState> externalFormKey;
  const ParcelDriverTipSelector({
    super.key,
    required this.externalFormKey,
  });

  @override
  State<ParcelDriverTipSelector> createState() =>
      _ParcelDriverTipSelectorState();
}

class _ParcelDriverTipSelectorState extends State<ParcelDriverTipSelector> {
  final RedirectController redirect = Get.put(RedirectController());
  int? selectedIndex;
  String? parcelselectedAmount;
  final TextEditingController customAmountController = TextEditingController();
  // Foodcartcontroller tipscontroller = Get.put(Foodcartcontroller());
  Foodcartcontroller tipscontroller = Get.find<Foodcartcontroller>();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final storedTip = //
          ...
          // normalizeTip(tipscontroller.parcelselectedTipAmount.value);
          normalizeTip(
              Provider.of<TipsProvider>(context, listen: false).tipamount);
      ...
      if (storedTip.isNotEmpty) {
        if (tipAmounts.contains(storedTip)) {
          setState(() {
            ...
            parcelselectedAmount =
                tipAmounts[selectedIndex!]; // normalized value
            customAmountController.clear();
          });
        } else {
          setState(() {
            selectedIndex = tipAmounts.length; // "Other" index
            parcelselectedAmount = storedTip;
                  ...
                  children: [
                    Wrap(
                      ...
                      children: List.generate(tipAmounts.length + 1, (index) {
                        ...
                        final String label =
                            isOther ? 'Other' : tipAmounts[index];
                        ...
                        return GestureDetector(
                          onTap: () {
                            setState(() {
                              if (isSelected) {
                                selectedIndex = null;
                                parcelselectedAmount = null;
                                customAmountController.clear();
                                //  tipscontroller.setparcelTipAmount(
                                tipsProvider.setTips(
                                    ''); // Clear stored tip when unselected
                              } else {
                                selectedIndex = index;
                                if (isOther) {
                                  parcelselectedAmount = null;
                                  ...
                                  // tipscontroller.setparcelTipAmount(
                                  tipsProvider.setTips(
                                      ''); // Clear stored tip when choosing "Other"
                                } else {
                                  parcelselectedAmount = tipAmounts[index];
                                  // tipscontroller.setparcelTipAmount(
                                  tipsProvider.setTips(
                                      parcelselectedAmount!); // Store selected tip
                                }
                              }
                            });
                          },
                    ...
                    if (selectedIndex == tipAmounts.length)
                      Padding(
                        ...
                        child: TextFormField(
                          ...
                          // onChanged: (value) {
                          //   setState(() {
                          //     parcelselectedAmount = value.trim();
                          //     tipscontroller.setTipAmount(parcelselectedAmount!); // Save in GetX
                          //   });
                          // },
//                           onChanged: (value) {
...
//     if (number != null) {
//       setState(() {
//         parcelselectedAmount = trimmedValue;
//         tipscontroller.setTipAmount(parcelselectedAmount!);
//       });
//     } else {
...
// },
                            ...
                            if (trimmedValue.isEmpty) {
                              ...
                              if (number != null) {
                                setState(() {
                                  parcelselectedAmount = trimmedValue;
                                  //   tipscontroller.setparcelTipAmount(
                                  tipsProvider.setTips(parcelselectedAmount!);
                                });
                              }
                            }
                          },
                          decoration: const InputDecoration(
                            ...
                            filled: true,
                            errorStyle:
                                CustomTextStyle.notetext, // Add this line
                    ...
                    // if (parcelselectedAmount != null && parcelselectedAmount!.isNotEmpty)
                    //   Text(
