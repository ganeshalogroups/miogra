...
import 'OrderScreenController/trackOrderController.dart';
...
class _TrackOrderDetailsState extends State<TrackOrderDetails> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _currentPosition = widget.resturantlatlng;
  ...
  void _addMarkers() async {
    ...
    markers.add(Marker(
      ...
      position: widget.resturantlatlng,
      icon: resturantIcon!,
...
if (!mounted) return;
  ...
  _timer = Timer.periodic(const Duration(seconds: 2), (timer) async {
    deliveryManPosition = deliveryManRef.child(widget.orderId).onValue.listen(
      (event) async {
  // DatabaseEvent event = await deliveryManRef.once();
        ...
        if (vehicleData != null) {
          ...
          if (vehicleData['latitude'] != null && vehicleData['longitude'] != null && vehicleData['heading'] != null) {
            ...
            if (!mounted) return;
// setState(() {
//   latt=vehicleData['latitude'].toString();
// });
 ...
 print("Latland:${LatLng(latitude, longitude)}");
            }
           }
        }
      },
...
// void startFirebaseUpdates() {
//   deliveryManPosition = deliveryManRef.child(widget.orderId).onValue.listen((event) async {
//     final vehicleData = event.snapshot.value as Map?;
//     if (vehicleData != null &&
//         vehicleData['latitude'] != null &&
//         vehicleData['longitude'] != null &&
//         vehicleData['heading'] != null) {
...
void _animateMarkerToPosition(LatLng newPosition) {
  // Dispose only if it's not already disposed and not animating
  if (_animationController != null && _animationController!.isAnimating) {
    _animationController!.stop();
  }
  _animationController?.dispose(); // Dispose safely
  ...
  //         _currentPosition = _latLngAnimation!.value;
  //         // Update the marker's position
  //         markers.removeWhere((m) => m.markerId == MarkerId('moving_marker'));
  //         markers.add(Marker(
...
//     });
//   });
// print('Animating marker to: $newPosition');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        child: const Text('Track Order', style: CustomTextStyle.darkgrey)),
        centerTitle: true,
      ),
      body: Stack(
        children: [
              ...
              SizedBox(
                  ...
                  child: GoogleMap(
                    ...
                    initialCameraPosition: CameraPosition(
                      ...
                      target: LatLng((widget.userlat.latitude + widget.resturantlatlng.latitude) / 2, (widget.userlat.longitude + widget.resturantlatlng.longitude) / 2),
                      zoom: 14,
                    ),
                  ...
                  //   },
                  onCameraMove: (position) {
  shouldFollowMarker = false; // User moved the camera manually
},
                                                              ...
                                                              child: Column(
                                                                ...
                                                                children: [
                                                                  Row(
                                                                    ...
                                                                    children: [
                                                                         Text(
                                                                          getStatusLabel(orderSts ?? ''),
                                                                          style: getStatusTextStyle(orderSts ?? ''),
  ...
  void prepareCurvedPolylines() {
    polylines.add(
        Polyline(
          ...
          points: MapsCurvedLines.getPointsOnCurve(widget.userlat, widget.resturantlatlng),
          color: Colors.black,
        )
    );
  }
...
if(_activeIndex >=3){
  ...
  // Create LatLngBounds between the delivery person and user location
      ...
      // );
if (shouldFollowMarker && _mapController != null) {
...
if(_activeIndex < 3 ){
  ...
  // Create LatLngBounds between the delivery person and user location
      LatLngBounds bounds = LatLngBounds(
        ...
        southwest: LatLng(
          min(widget.resturantlatlng.latitude, widget.userlat.latitude),
          min(widget.resturantlatlng.longitude, widget.userlat.longitude),
        ),

        northeast: LatLng(
          max(widget.resturantlatlng.latitude, widget.userlat.latitude),
          max(widget.resturantlatlng.longitude, widget.userlat.longitude),
        ),
  ...
  List<StepperData> buildStepperData() {
          return [
            StepperData(
              title: StepperText(
                ...
                textStyle: _activeIndex >= 1 ? //CustomTextStyle.DECORATION_regulartext 
                TextStyle(
      fontSize: 13,
      ...
      color: Customcolors.darkpurple
              ...
              StepperData(
                title: StepperText(
                  "Order picked up",
                  textStyle: _activeIndex >= 3 ?  TextStyle(
      fontSize: 13,
      fontWeight: FontWeight.w500,
      fontFamily: 'Poppins-Medium',
      color: Customcolors.darkpurple
      ...
      color: Customcolors.darkpurple
              ...
              StepperData(
                title: StepperText(
                  "Order delivered",
                  textStyle: _activeIndex >= 5 ?  TextStyle(
      fontSize: 13,
      fontWeight: FontWeight.w500,
      fontFamily: 'Poppins-Medium',
      color: Customcolors.darkpurple
